...
> [!IMPORTANT]
> 当前提供 `v0.2.0-beta.2` 桌面预览：Mac DMG、Windows 安装 EXE 和 Debian/Ubuntu DEB。
...
旧版本的登录要求以其发布说明为准。项目默认保存在 `~/TakeBoardData`；当前开发版支持在[所选设备的其他文件夹](docs/device-workflow.md)创建项目。升级前请先备份。

当前开发版也提供[统一设置与更新提醒](docs/settings-and-updates.md)：保存设备默认项目位置、调整外观，以及检查适合当前电脑的新版安装包；不自动安装或重启。
