...
服务已启用且当前运行，但该账户的 systemd `Linger=no`。如需在服务器重启后、不登录 SSH 的情况下自动启动，需要在 4090 上手工输入一次 sudo 密码：

```bash
sudo loginctl enable-linger duanqw
```
...
随后又通过 TakeBoard 自身 API 完成第二次端到端生成：45.54 秒，输出 2,003,834 bytes，并成功形成 Run、Asset、Take 和 Approval。精简 Recipe 执行时观测到约 23.2GB 显存占用，已接近 24GB 上限，因此首版保持单任务串行，暂不提高分辨率或并发。

当前根盘使用率约 85%（尚余约 134GB）。视频项目会持续消耗空间，进入多用户试用前需要增加项目级容量提示与清理/归档功能。
