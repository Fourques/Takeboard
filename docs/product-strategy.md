# TakeBoard 产品策略与完整立项论证

> 暂定名：正式公开前还需要做商标、域名和同名项目检索。

状态：M0 可用纵向切片；已跑通自托管 ComfyUI 的真实项目链路。

> 本文档保留 TakeBoard 的完整产品判断、竞品定位、验证门槛与商业化思路。面向 GitHub 访客的项目介绍见 [项目首页](../README.md)；准备开工请从 [START-HERE](../START-HERE.md) 开始。

> 当前版本已完成项目主页、角色与场景资产入库、语义连线、ComfyUI Workflow 自动检测/导入、文生图/图生图、I2V 与首尾帧、运行轮询、图片/视频回收和 Take 入库。部署方式见 [自托管指南](self-hosting.md)，图片模型见 [Qwen-Image-2512](qwen-image-2512.md)，创作界面见 [创作工作站指南](creator-workstation.md)；无计算资源时也可打开功能示例。
...
M0 具体建议采用 TypeScript monorepo、React/Vite、React Flow、Fastify、SQLite/Drizzle、Zod 和 Playwright；理由、数据边界与安全约束见 [技术架构](architecture.md)。
...
- [MVP 产品需求](mvp-prd.md)
- [技术架构与数据边界](architecture.md)
- [架构与产品决策记录](decisions.md)
- [8 周路线与 Issue-ready Backlog](roadmap.md)
- [竞品、访谈和可用性验证](research-plan.md)
