...
- Release 的便携包附带同名 `.sha256` 文件。校验和用于核对完整性，不代替发布者身份验证或安全审计。
  熟悉 GitHub CLI 的用户可对支持来源证明的构建执行 `gh attestation verify <下载文件路径> --repo Fourques/Takeboard`。
...
后续发布应明确标记预览 / 正式、源代码提交、平台、签名状态、校验文件、升级说明和已知限制。
