...
- `canvas.resize_item` 保留为旧客户端兼容命令，参数为 `itemId`、`x`、`y`、`width`、`height`。当前画布不提供自由拉伸控制点：旧的 `sizeMode: manual` 在显示时采用稳定宽度和内容自然高度，不应用历史手动高度，不裁切或改写媒体文件；存储值仍保留，避免破坏旧客户端及操作历史。
