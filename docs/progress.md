...
更新时间：2026-08-13
当前工作点：真实本地生成闭环已完成；下一步是 Run 恢复、四候选批量与首个图片模型 Recipe。
...
| `TB-010` 真实生成 | 完成 | 提交、History、输出回收、失败收敛；Wan I2V/FLF 真实验证 |

## 当前质量基线

- 自动测试：47 个（契约、领域、Recipe、执行器、服务）；
- 领域规则测试：5 个；
- 服务端测试：6 个；
- Playwright 黄金路径：2 个，真实 Chromium 完整通过；
- `pnpm verify`：通过；
- `playwright test`：通过，覆盖重置、生成、淘汰、批准和刷新恢复。

## 下一纵向切片

先完成服务重启后的 Run 自动对账和“一组四候选”批量提交，再根据实际创作需求安装并冻结一个图片
checkpoint，补一套 T2I/I2I 官方 Recipe。Fake Executor 继续作为 CI 和无 GPU Demo。
