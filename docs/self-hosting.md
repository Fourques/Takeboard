...
服务端会持续核对未完成的生成并回收结果，不依赖浏览器保持打开；关闭页面不等于取消生成。
如执行端队列和历史均找不到任务，会先等待至少 30 秒再标记“状态待确认”，不会虚报完成。
取消请求只有在执行端确认任务退出队列后才算停止；无法确认时保留项目与追踪记录。

同一数据目录只允许一个服务进程使用。桌面、便携与源码启动器通过 `.system/instance.json` 发现已有实例，
服务端通过 `.system/instance-lock.db` 的跨进程锁阻止重复写入，离线恢复也使用同一把锁。
不要在运行中删除或替换这个锁文件；进程退出或崩溃后，系统会释放锁。
项目数据目录应放在支持可靠文件锁的本地磁盘；跨机器协作请连接同一个服务，不要让多台机器直接共享写数据库。

`TAKEBOARD_AUTH_DATABASE` 是完整的数据库文件路径。旧版误创建的 `auth.db/.system/auth.db` 会继续读取原位置，
离线恢复使用同一解析规则，不会自动创建空账号库或丢弃旧数据。
...
日常也可以由实例管理员打开“账号 → 备份与恢复”，创建带双层完整性校验的实例备份。它包含全部项目、素材、生成结果和身份数据库；生成任务未结束时会拒绝创建。界面可以验证并在线补回缺失项目；需要连账号与权限一起恢复时，先停止服务再执行：

```bash
npm run easy:stop
npm run easy:restore -- ~/Downloads/<backup>.takeboard-instance.tgz --confirm
npm run easy
```
...
首次打开页面时创建的账号是实例管理员，升级前已有的项目会自动归属于该账号。管理员可以在右上角账号中心创建一次性团队邀请，新成员通过链接自行设置密码。项目 Owner 可以在项目内账号中心设置成员角色：

- `Owner`：完整编辑、成员管理、导出与删除；
- `Editor`：创作、素材管理、项目重命名和生成，但不能导出、管理成员或删除项目；
- `Viewer`：只读查看项目、素材和运行结果。

实例管理员可以为故障恢复访问所有项目和回收站，界面以 `ADMIN ACCESS` 标识，不冒充项目 Owner。权限在服务端逐个 API 校验，隐藏按钮只用于改善体验，不是安全边界。密码修改会撤销其他设备会话；账号停用会立即撤销该成员的全部会话。数据库位置、备份和恢复方式见[账号与权限说明](./access-control.md)。
