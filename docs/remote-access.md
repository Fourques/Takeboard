...
桌面菜单“连接 → 连接设备…”（Mac：`⌘⇧K`，Windows/Linux：`Ctrl+Shift+K`）提供 SSH、HTTPS 和 Portal 三个入口。连接成功后保存地址与实例标识，下次从最近连接打开；账号 Cookie 仍由对应服务管理，不保存 SSH 密码。远程页面在不具备桌面命令权限的独立窗口打开。关闭该窗口会释放本次连接，不会停止远端生成。

SSH 入口使用系统 OpenSSH 客户端、已配置密钥和已确认的主机指纹；支持 SSH 别名中的自定义 SSH 端口或跳板机。尚未配置密钥/可信主机时会明确失败，不会偷偷忽略指纹，也不提供假密码输入框。默认自动检测服务端 `48120–48139`；不在此范围的自定义或随机端口需要在“自定义服务端口”填写。HTTPS 入口只接受 HTTPS 服务源地址，HTTP 仅允许既有本机隧道地址。
