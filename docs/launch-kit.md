...
> 我做了 TakeBoard：一个给 ComfyUI 创作者使用的开源、本地优先 AI 影像工作台。它不重做节点编辑器，而是补上项目层——素材、镜头、首尾帧/参考输入、Workflow、每次 Run、候选 Take、批准结果和只读粗剪都在同一张画布里。
>
> 项目和媒体保存在自己的机器；自定义 Workflow 不会因为文件名相似就直接执行，而是先诊断依赖、建立参数 Binding，并用内容哈希锁定信任。生成进度只在 ComfyUI 提供真实节点信息时显示百分比，未知时明确保持不确定状态。
>
> v0.1.0 是 Public Preview，适合个人创作者和可信自托管团队，不宣称是零配置桌面软件或公网 SaaS。现在最希望找到几位有真实视频 Workflow 的创作者，测试“导入 → 映射 → 生成 → 选片 → 粗剪”全过程。欢迎直接反馈阻断点；运行中心可以导出不含素材、提示词、账号和路径的诊断报告。
>
...
6. 70–82s：打开分镜墙与粗剪，显示未完成镜头的计划空镜。
